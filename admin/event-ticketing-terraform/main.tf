...
# 5. Security Groups (Strict Chaining)
...
resource "aws_security_group" "ec2_app_sg" {
  ...
  description = "Inbound HTTP strictly from ALB SG"
  vpc_id      = aws_vpc.custom_vpc.id
...
resource "aws_security_group" "rds_sg" {
  name        = "tarumt-rds-sg"
  description = "Inbound MySQL strictly from EC2 App SG"
  vpc_id      = aws_vpc.custom_vpc.id
...
# 7. RDS MySQL Instance (Single AZ, Private Subnet)
...
# 9. Launch Template (Using AWS Academy Pre-Existing LabInstanceProfile)
...
resource "aws_launch_template" "app_lt" {
  ...
  # AWS Academy Pre-Existing Instance Profile
  ...
  user_data = base64encode(<<-EOF
              #!/bin/bash
              dnf update -y
              dnf install -y httpd php php-mysqlnd php-json php-mbstring git
              systemctl start httpd
              systemctl enable httpd
              echo "<?php http_response_code(200); echo 'HEALTHY'; ?>" > /var/www/html/healthz.php
